...
public class PongGame extends JPanel implements MouseMotionListener{
    ...
    static int bounceCount = 0, futureY = -1;
    ...
    public void paintComponent(Graphics g){
        ...
        // update score
        g.setColor(Color.WHITE);
        g.drawString("Score \nUser[ " + userScore + " ]     \tPC[ " + pcScore + " ]", 250, 20);
        // draw the components
        gameBall.paint(g);
        //futureBall.paint(g);
        userPaddle.paint(g);
    ...
    public void gameLogic(){
        ...
        if(futureY == -1){
            for(int i = 0; i < 10; i++){
                ...
                // if future ball collides with user paddle reverse x dir
                if(futureBall.getX() < userPaddle.getX() + Paddle.PADDLE_WIDTH){
                    futureBall.reverseX();
                }

                // if future ball goes past pc paddle x, we know this is where paddle needs to go
                if(futureBall.getX() > pcPaddle.getX()){
                    futureY = futureBall.getY();
                    break;   
                }
            }
        }
